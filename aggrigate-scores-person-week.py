...
def aggregate_sentiment_scores_by_week_and_recipient(input_excel_path, output_excel_path):
    ...
    with pd.ExcelWriter(output_excel_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
        for recipient in recipients:
            ...
            aggregated_scores = recipient_df.groupby('Year-Week')['Classified Sentiment'].sum().reset_index()
...
def main():
    ...
    output_excel_path = 'messages_with_sentiment.xlsx'  # Output file with a different name to reflect weekly aggregation
